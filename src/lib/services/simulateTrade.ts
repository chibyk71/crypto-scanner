...
/**
 * Minimum risk distance (% of entry) used when computing R-multiples.
 * 0.0001 (0.01%) previously produced |R| > 50 on 79/81 baseline rows.
 * 0.001 (0.1%) is a more realistic minimum SL distance for crypto.
 * Forward-looking only — historical rows are not rewritten.
 */
export const MIN_RISK_PCT_FOR_R = 0.001;

/**
 * Compute risk percentage of entry for R-multiple denominator.
 * Exported for unit tests; used by storeAndFinalizeSimulation.
 */
export function computeRiskPctForR(
    entryPrice: number,
    stopLoss: number | undefined | null
): number {
    if (stopLoss && stopLoss > 0 && entryPrice > 0) {
        const riskPct = Math.abs(entryPrice - stopLoss) / entryPrice;
        return Math.max(riskPct, MIN_RISK_PCT_FOR_R);
    }
...
async function storeAndFinalizeSimulation(params: {
    ...
    const riskPct = computeRiskPctForR(entryPrice, signal.stopLoss);
